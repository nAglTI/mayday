...
class DefaultVpnProfileRepository @Inject constructor(
    ...
    private fun readRelays(preferences: Preferences): List<VpnRelayTarget> {
                    ...
                    for (index in 0 until array.length()) {
                        ...
                        add(
                            VpnRelayTarget(
                                ...
                                ports = item.optPorts().ifEmpty {
                                    addr.parseRelayAddress()?.second?.let(::listOf).orEmpty()
                                },
                            ),
                        )
                    }
        ...
        return listOf(
            VpnRelayTarget(
                ...
                shortId = 1,
                ports = listOf(preferences[RELAY_PORT] ?: 443),
            ),
        )
    }
    ...
    private fun JSONObject.optPorts(): List<Int> {
        val ports = optJSONArray("ports") ?: return emptyList()
        return buildList {
    ...
    private fun VpnRelayTarget.portCandidates(addr: String): List<Int> {
        ...
        return addr.parseRelayAddress()?.second?.let(::listOf).orEmpty()
    }
